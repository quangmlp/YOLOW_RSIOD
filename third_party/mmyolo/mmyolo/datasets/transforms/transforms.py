        ...
        for instance in results.get('instances', []):
            if instance['ignore_flag'] == 0:
                if 'mask' in instance:
                    ...
                    # Actually, gt with bbox and without mask needs to be retained
                    # We can construct a dummy mask from the bbox
                    bbox = instance.get('bbox', None)
                    if bbox is not None and len(bbox) == 4:
                        x1, y1, x2, y2 = bbox
                        dummy_polygon = [x1, y1, x2, y1, x2, y2, x1, y2]
                        gt_masks.append([np.array(dummy_polygon)])
                        gt_ignore_flags.append(instance['ignore_flag'])
                        self._mask_ignore_flag.append(1)
                    else:
                        self._mask_ignore_flag.append(0)
