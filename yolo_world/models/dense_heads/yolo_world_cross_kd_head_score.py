...
from yolo_world.models.dense_heads.yolo_world_head import BNContrastiveHead, ContrastiveHead
...
def gt_instances_preprocess(batch_gt_instances: Union[Tensor, Sequence],
    ...
    if isinstance(batch_gt_instances, Sequence):
        ...
        box_dim = batch_gt_instances.size(-1) - 3
        if len(batch_gt_instances) > 0:
            ...
            batch_instance = torch.zeros(
                (batch_size, max_gt_bbox_len, box_dim + 2),
            ...
            batch_instance = torch.zeros((batch_size, 0, box_dim + 2),
                                         dtype=batch_gt_instances.dtype,
                                         device=batch_gt_instances.device)
...
class YOLOWorldCrossKdScoreHeadModule(YOLOv8HeadModule):
    ...
    def forward_with_feat(self, img_feats: Tuple[Tensor],
                txt_feats: Tensor) -> Tuple[List]:
        """Forward features from the upstream network."""
        assert len(img_feats) == self.num_levels
        txt_feats = [txt_feats for _ in range(self.num_levels)]
        return multi_apply(self.forward_single_with_feat, img_feats, txt_feats,
                           self.cls_preds, self.reg_preds, self.cls_contrasts)
    ...
    def forward_single_with_feat(self, img_feat: Tensor, txt_feat: Tensor,
                       cls_pred: nn.ModuleList, reg_pred: nn.ModuleList,
                       cls_contrast: nn.ModuleList) -> Tuple:
        """Forward feature of a single scale level."""
        b, _, h, w = img_feat.shape
        ...
        if self.reg_max > 1:
            ...
            bbox_preds = bbox_dist_preds
        if self.training:
            return cls_embed, cls_logit, bbox_preds, bbox_dist_preds
        else:
            return cls_logit, bbox_preds


@MODELS.register_module()
class YOLOWorldCrossKdScoreHead(YOLOv8Head):
    """YOLO-World Head
    """
    ...
    def loss_by_feat(
        ...
        assigned_result = self.assigner(
            ...
            gt_labels, gt_bboxes, gt_scores, pad_bbox_flag)
    ...
    def forward_with_feat(self, img_feats: Tuple[Tensor], txt_feats: Tensor,
                          batch_data_samples: Union[list, dict]) -> dict:

        outs = self.head_module.forward_with_feat(img_feats, txt_feats)
        # Fast version
        loss_inputs = outs + (batch_data_samples['bboxes_labels'],
                              batch_data_samples['img_metas'])
        return self.loss_with_feat(*loss_inputs)

    def loss_with_feat(
            self,
            cls_embeds: Sequence[Tensor],
            cls_scores: Sequence[Tensor],
            bbox_preds: Sequence[Tensor],
            bbox_dist_preds: Sequence[Tensor],
            batch_gt_instances: Sequence[InstanceData],
        ...
        flatten_cls_preds = torch.cat(flatten_cls_preds, dim=1)
        flatten_pred_bboxes = torch.cat(flatten_pred_bboxes, dim=1)
        flatten_cls_embeds = torch.cat(flatten_cls_embed, dim=1)
        flatten_pred_bboxes = self.bbox_coder.decode(
            self.flatten_priors_train[..., :2], flatten_pred_bboxes,
            self.stride_tensor[..., 0])
        ...
        return flatten_cls_embeds, flatten_cls_preds, flatten_pred_bboxes
