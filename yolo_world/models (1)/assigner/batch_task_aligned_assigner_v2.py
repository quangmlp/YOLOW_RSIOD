...
class BatchTaskAlignedScoreV2Assigner(nn.Module):
    ...
    def forward(
        ...
        assigned_result = {
            ...
            'assigned_weight':
                gt_bboxes.new_full((pred_scores.shape[0], pred_scores.shape[1], 1), 0),
        }
        ...
        # assigned target
        assigned_labels, assigned_bboxes, assigned_scores, assigned_weight = self.get_targets(
            gt_labels, gt_bboxes, gt_scores, assigned_gt_idxs, fg_mask_pre_prior,
            batch_size, num_gt)
        ...
        assigned_weight = assigned_weight.unsqueeze(-1)
        assigned_scores = assigned_scores * assigned_weight
    ...
    def get_targets(self, gt_labels: Tensor, gt_bboxes: Tensor, gt_weight: Tensor,
                    assigned_gt_idxs: Tensor, fg_mask_pre_prior: Tensor,
                    batch_size: int,
                    num_gt: int) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
        """Get assigner info.
        ...
        assigned_weight = gt_weight.float().flatten()[assigned_gt_idxs]  # → shape: [16, 8400]

        return assigned_labels, assigned_bboxes, assigned_scores, assigned_weight
