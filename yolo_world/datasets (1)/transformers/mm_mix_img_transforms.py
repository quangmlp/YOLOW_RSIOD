...
class MultiModalMosaic(BaseMultiModalMixImageTransform):
    ...
    def mix_img_transform(self, results: dict) -> dict:
        ...
        for i, loc in enumerate(loc_strs):
            ...
            gt_bboxes_score_i = None
            if 'gt_bboxes_scores' in results_patch:
                gt_bboxes_score_i = results_patch['gt_bboxes_scores']
            ...
            if gt_bboxes_score_i is not None:
                mosaic_bboxes_scores.append(gt_bboxes_score_i)
        ...
        if gt_bboxes_score_i is not None:
            mosaic_bboxes_scores = np.concatenate(mosaic_bboxes_scores, 0)
        ...
        if self.bbox_clip_border:
            ...
            if gt_bboxes_score_i is not None:
                mosaic_bboxes_scores = mosaic_bboxes_scores[inside_inds]
        ...
        if gt_bboxes_score_i is not None:
            results['gt_bboxes_scores'] = mosaic_bboxes_scores
...
class YOLOv5MultiModalMixUp(BaseMultiModalMixImageTransform):
    ...
    def mix_img_transform(self, results: dict) -> dict:
        ...
        retrieve_gt_bboxes_scores = None
        if 'gt_bboxes_scores' in retrieve_results:
            retrieve_gt_bboxes_scores = retrieve_results['gt_bboxes_scores']
        ...
        if retrieve_gt_bboxes_scores is not None:
            mixup_gt_bboxes_scores = np.concatenate(
                (results['gt_bboxes_scores'], retrieve_gt_bboxes_scores), axis=0)
        ...
        if retrieve_gt_bboxes_scores is not None:
            results['gt_bboxes_scores'] = mixup_gt_bboxes_scores
